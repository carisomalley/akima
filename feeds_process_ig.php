 ...
 * These can be pasted directly into Unsplash/Pexels/Pixabay.
 */
function getSearchSuggestions(string $title, string $summary, string $key): array {
    $resp = chatgpt_request([
      ...
      'messages' => [
        ...
        [
          ...
          . "should be 1–2 words long and reflect imagery from the article."
        ]
      ],
      'max_tokens' => 100,
      'temperature' => 0.7
    ...
    // Split GPT’s response on commas or newlines, strip numbering
    $terms = preg_split('/[,\r\n]+/', $resp);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  // d) RF search: collect 'thumb' + 'full' URLs
  elseif (isset($_POST['rf_site'])) {
      ...
      if (!$q) {
          ...
          switch ($_POST['rf_site']) {
              case 'unsplash':
                  if ($unsplashKey) {
                      ...
                      foreach ($d['results'] ?? [] as $img) {
                          $rf_results[] = [
                            'thumb' => $img['urls']['small'],  // preview
                            'full'  => $img['urls']['full'],   // download
                          ];
                      }
                  } else {
              ...
              case 'pexels':
                  if ($pexelsKey) {
                      ...
                      foreach ($d['photos'] ?? [] as $img) {
                          $rf_results[] = [
                            'thumb' => $img['src']['medium'],  // preview
                            'full'  => $img['src']['large'],   // download
                          ];
                      }
                  } else {
              ...
              case 'pixabay':
                  if ($pixabayKey) {
                      $u2 = "https://pixabay.com/api/?key={$pixabayKey}&q=" . urlencode($q) . "&per_page=6";
                      $d  = @json_decode(file_get_contents($u2), true);
                      foreach ($d['hits'] ?? [] as $img) {
                          $rf_results[] = [
                            'thumb' => $img['previewURL'],        // preview
                            'full'  => $img['largeImageURL'],     // download
                          ];
                      }
                  } else {
  ...
  // e) INITIAL: generate IG post + hashtags + prompts + suggestions
  elseif ($stage === 'initial') {
      if ($summary === '') {
          $error = 'Please edit the summary before generating.';
      }
      if (!$error) {
          // 1) Post + hashtags — system prompt for Instagram
          $sys = [
            'role' => 'system',
            'content' =>
              "You are a social-media assistant specialized in crafting Instagram posts.\n"
          ...
          $usr = [
            ...
            . "Tone: {$tone}\n\n"
            . "Write the Instagram post accordingly, then append hashtags."
          ];
          $resp = chatgpt_request([
            'model' => 'gpt-3.5-turbo',
            'messages' => [$sys, $usr],
  ...
  elseif (isset($_POST['publish_ig'])) {
      if (!$instaAccessToken || !$instaUserId) {
          ...
          // Instagram Graph API: if only one image, create single-media container
              ...
              if (curl_errno($ch)) {
                  ...
                  if (isset($j['id'])) {
                      ...
                      if (curl_errno($ch2)) {
                          ...
                          if (isset($j2['id'])) {
                              $successMessage = "Posted to Instagram successfully (Media ID: {$j2['id']}).";
                          } else {
                              $error = 'Instagram publish failed: ' . ($j2['error']['message'] ?? $resp2);
                          }
                      }
                  } else {
          ...
          // If multiple images, create a carousel
          elseif (count($absoluteUrls) > 1) {
              // 2a) Create individual media children
              $childrenIds = [];
              ...
              // 2b) If no error so far, create the carousel container
              if (!$error) {
                  $endpoint = "https://graph.facebook.com/{$instaUserId}/media";
                  $postFields = [
                      'caption'        => $caption,
                  ...
                  if (curl_errno($ch2)) {
                      ...
                      if (isset($j2['id'])) {
                          ...
                          if (curl_errno($ch3)) {
                              ...
                              if (isset($j3['id'])) {
                                  $successMessage = "Posted carousel to Instagram successfully (Media ID: {$j3['id']}).";
                              } else {
...
<body class="container py-4">
  <h1 class="h4 mb-3">Instagram Post Builder</h1>
  ...
  <?php if ($stage === 'initial'): ?>
    <form method="post" class="mb-4">
      ...
      <button class="btn btn-primary">Generate Instagram Post</button>
  ...
  <?php if ($stage === 'choose_image'): ?>
    ...
    <!-- Selected Images Gallery (with Download via download.php) -->
    ...
    <?php if ($searchSuggestions): ?>
      <div class="mb-3">
        ...
        <?php foreach ($searchSuggestions as $t): ?>
          <button type="button" class="btn btn-sm btn-outline-info suggestion-btn"><?= $t ?></button>
        <?php endforeach; ?>
      </div>
      <script>
    ...
    <!-- RF results (use thumb for preview, full for “+”) -->
    <?php if ($rf_results): ?>
      <div class="row g-2 mb-4">
        <?php foreach ($rf_results as $imgInfo):
          $thumbUrl = $imgInfo['thumb'];
          $fullUrl  = $imgInfo['full'];
          if (!in_array($fullUrl, $selectedImages)): ?>
            <div class="col-4 col-md-2">
              <div class="thumb-container position-relative">
                <img src="<?= htmlspecialchars($thumbUrl) ?>" class="img-fluid rounded">
                <form method="post" style="position:absolute; top:5px; right:5px; z-index:10;">
                  <input type="hidden" name="item_id"    value="<?= $itemId ?>">
                  <input type="hidden" name="stage"      value="choose_image">
                  <input type="hidden" name="rf_add_url" value="<?= htmlspecialchars($fullUrl) ?>">
    ...
    <div class="mt-4">
      <form method="post" style="display:inline-block;">
        <input type="hidden" name="item_id" value="<?= $itemId ?>">
        <input type="hidden" name="stage"   value="choose_image">
        <button name="publish_ig" value="1" class="btn btn-success">
